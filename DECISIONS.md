...
- **Role of `v_promotable` in Lifecycle Rehearsal**: Because candidate `v0002` was legitimately and correctly rejected by production governance, `v0002` was never promoted to production. To enable end-to-end rehearsal and operational demonstration of the atomic rollback machinery (`make rollback`) without falsifying promotion governance or overriding holdout rejection rules, the repository includes a committed, deterministic model package fixture: **`v_promotable`** (`models/v_promotable`). `v_promotable` shares the weighted multi-signal architecture of `v0002` but is backed by committed evaluation evidence that clears all four promotion criteria (15.49% development improvement and directional agreement on the holdout: 17 → 14 missed weeks). This cleanly decouples the demonstration of rollback safety (pre-validation, atomic pointer switch, bit-for-bit replay proof, compensating transaction) from the empirical fate of any specific experimental candidate.
